...
export default function LegalPage() {
  ...
  const handleSave = async () => {
    try {
      if (editing) { await legalApi.update(editing.id, form); toast.success("تم التحديث"); }
      else { await legalApi.create(form); toast.success("تم الإنشاء"); }
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await legalApi.delete(id); toast.success("تم الحذف"); load(); }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-white">الصفحات القانونية</h1><p className="text-sm text-gray-400">{pages.length} صفحة</p></div>
        <button onClick={openCreate} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700"><Plus className="h-4 w-4" /> إضافة صفحة</button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-800">
        <table className="w-full text-sm">
          <thead className="bg-gray-900 text-gray-400">
            <tr>
              <th className="px-4 py-3 text-right font-medium">المعرّف</th>
              <th className="px-4 py-3 text-right font-medium">العنوان</th>
              <th className="px-4 py-3 text-right font-medium">العربية</th>
              <th className="px-4 py-3 text-right font-medium">الكردية</th>
          ...
          <tbody className="divide-y divide-gray-800">
            {pages.map((p: any) => (
              <tr key={p.id} className="hover:bg-gray-900/50">
                ...
                <td className="px-4 py-3">{p.isActive ? <span className="rounded-full bg-green-600/20 px-2 py-0.5 text-xs text-green-400">نشط</span> : <span className="rounded-full bg-gray-700 px-2 py-0.5 text-xs text-gray-400">غير نشط</span>}</td>
        ...
        {pages.length === 0 && <p className="p-8 text-center text-gray-500">لا توجد صفحات قانونية</p>}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl border border-gray-800 bg-gray-900 p-6">
            <div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold text-white">{editing ? "تعديل الصفحة" : "إضافة صفحة"}</h2><button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-white"><X className="h-5 w-5" /></button></div>
            <div className="space-y-3">
              <div><label className="mb-1 block text-xs text-gray-400">المعرّف</label><input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} placeholder="مثلاً privacy-policy" className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none font-mono" /></div>
              ...
              <div><label className="mb-1 block text-xs text-gray-400">المحتوى (EN)</label><textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={4} className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none resize-none" /></div>
              <div><label className="mb-1 block text-xs text-gray-400">المحتوى (AR)</label><textarea value={form.contentAr} onChange={(e) => setForm({ ...form, contentAr: e.target.value })} rows={4} className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none resize-none" dir="rtl" /></div>
              <div><label className="mb-1 block text-xs text-gray-400">المحتوى (KU)</label><textarea value={form.contentKu} onChange={(e) => setForm({ ...form, contentKu: e.target.value })} rows={4} className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none resize-none" dir="rtl" /></div>
