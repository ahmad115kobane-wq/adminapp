...
export default function UsersPage() {
  ...
  const handleToggleBan = async (id: string) => {
    try { const res = await adminApi.toggleBan(id); toast.success(res.data.message || "تم"); load(); }
    catch { toast.error("فشل"); }
  };

  const handleRoleChange = async (id: string, role: string) => {
    try { await adminApi.updateRole(id, role); toast.success("تم تحديث الدور"); load(); }
    catch (err: any) { toast.error(err.response?.data?.message || "فشل"); }
  };

  const handleDelete = async (id: string) => {
    if (!confirm("حذف هذا المستخدم نهائياً؟")) return;
    try { await adminApi.deleteUser(id); toast.success("تم حذف المستخدم"); load(); }
    catch (err: any) { toast.error(err.response?.data?.message || "فشل"); }
  };
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400">{total} مستخدم</p>
      </div>

      <div className="mb-4 flex flex-wrap items-center gap-3">
        <div className="flex flex-1 items-center gap-2 rounded-lg border border-gray-800 bg-gray-900 px-3 py-2">
          <Search className="h-4 w-4 text-gray-500" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="بحث بالاسم أو البريد..." className="flex-1 bg-transparent text-sm text-white outline-none placeholder-gray-500" />
        </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-800">
          ...
          <tbody className="divide-y divide-gray-800">
            {loading ? (
              ...
              <tr key={u.id} className="hover:bg-gray-900/50">
                ...
                <td className="px-4 py-3">
                  {u.isBanned ? <span className="rounded-full bg-red-600/20 px-3 py-1 text-xs font-medium text-red-400">محظور</span> : <span className="rounded-full bg-green-600/20 px-3 py-1 text-xs font-medium text-green-400">نشط</span>}
                </td>
        ...
        {!loading && users.length === 0 && <p className="p-8 text-center text-gray-500">لا يوجد مستخدمون</p>}
      </div>

      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between">
          <p className="text-sm text-gray-400">صفحة {page} من {totalPages}</p>
