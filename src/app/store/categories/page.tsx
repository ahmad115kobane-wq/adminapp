...
export default function CategoriesPage() {
  ...
  const handleSave = async () => {
    try {
      if (editing) { await storeApi.updateCategory(editing.id, form); toast.success("تم التحديث"); }
      else { await storeApi.createCategory(form); toast.success("تم الإنشاء"); }
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await storeApi.deleteCategory(id); toast.success("تم الحذف"); load(); }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-white">أقسام المتجر</h1><p className="text-sm text-gray-400">{categories.length} قسم</p></div>
        <button onClick={openCreate} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700"><Plus className="h-4 w-4" /> إضافة قسم</button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-800">
          ...
          <tbody className="divide-y divide-gray-800">
            {categories.map((c: any) => (
              <tr key={c.id} className="hover:bg-gray-900/50">
                ...
                <td className="px-4 py-3">{c.isActive ? <span className="rounded-full bg-green-600/20 px-2 py-0.5 text-xs text-green-400">نشط</span> : <span className="rounded-full bg-gray-700 px-2 py-0.5 text-xs text-gray-400">غير نشط</span>}</td>
        ...
        {categories.length === 0 && <p className="p-8 text-center text-gray-500">لا توجد أقسام</p>}
      </div>

      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-xl border border-gray-800 bg-gray-900 p-6">
            <div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold text-white">{editing ? "تعديل القسم" : "إضافة قسم"}</h2><button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-white"><X className="h-5 w-5" /></button></div>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="mb-1 block text-xs text-gray-400">الأيقونة</label><input value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none" /></div>
                <div><label className="mb-1 block text-xs text-gray-400">الترتيب</label><input type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: Number(e.target.value) })} className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none" /></div>
              </div>
