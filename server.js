...
function requireAuth(req, res, next) {
    ...
    const result = auth.verify(token, getClientIp(req));
    if (!result.valid) {
...
app.post('/api/admin/users', requireAdmin, (req, res) => {
    ...
    const userResult = db.createUser(username, password, max_devices || 1, notes || '');
    ...
    if (plan_id) {
        const subResult = db.createSubscription(userResult.userId, plan_id);
...
app.post('/api/admin/users/:id/block', requireAdmin, (req, res) => {
    const result = db.blockUser(req.params.id, true);
    res.json(result);
});
app.post('/api/admin/users/:id/unblock', requireAdmin, (req, res) => {
    const result = db.blockUser(req.params.id, false);
    res.json(result);
});

// إنشاء اشتراك لمستخدم
app.post('/api/admin/users/:id/subscribe', requireAdmin, (req, res) => {
    const { plan_id } = req.body;
    if (!plan_id) {
        return res.status(400).json({ success: false, error: 'حدد الباقة' });
    }
    const result = db.createSubscription(req.params.id, plan_id);
    res.json(result);
});

// قطع اتصال مستخدم (كل أجهزته)
app.post('/api/admin/users/:id/kick', requireAdmin, (req, res) => {
    const result = db.killAllUserSessions(req.params.id);
    res.json(result);
});

// قائمة الباقات
app.get('/api/admin/plans', requireAdmin, (req, res) => {
    res.json({ success: true, plans: db.getAllPlans() });
});

// سجل النشاط
app.get('/api/admin/activity', requireAdmin, (req, res) => {
    const limit = Math.min(parseInt(req.query.limit) || 100, 500);
    res.json({ success: true, log: db.getActivityLog(limit) });
});
...
app.post('/api/license/verify', (req, res) => {
    ...
    const result = db.verifyLicense(license_key);
...
app.post('/api/license/auth', (req, res) => {
    ...
    const sub = db.checkSubscriptionValid(user.id);
    if (!sub.valid) {
...
function startCleanupScheduler() {
    ...
    setInterval(() => {
        try {
            db.cleanExpiredSubscriptions();
            db.cleanStaleSessions();
        } catch (e) {
